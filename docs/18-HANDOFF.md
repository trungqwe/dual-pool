## Phase 2 — v7.3.8 candidate stage/install ACL seam repair (2026-09-22)

Repair run starts at `a4bc87bc4370a8010a8619ade147f53d3689eaed` on `phase-2/v7.3.8-candidate-stage-install`; authoritative `phase-2/upstream-lifecycle` remains `5bfb1518059a171a0f3858877585e3097d9fe099`. The blocker was a mismatch: StageCandidate creates source-stage children with normal filesystem APIs, while candidate installation incorrectly required the child directory and files to have protected installed-object ACLs. A RED regression reproduced the rejection; after the repair it passes with a protected `<Bin>\\upstream-stage` root, normal-API source children, and protected/revalidated installed destination. Public InstallCandidate derives the exact reviewed candidate directory through the shared `upstreamstage.CandidateStagePath` helper and rejects caller paths outside it. The private synthetic install engine remains available for component tests. `validateInstallFor` stays strict.

Targeted suites, four package stress ×50, four package race ×10, full Go tests/race, vet, module verification, Windows build, Node 55/55 and pinned-lock validation passed. The opt-in real v7.3.8 archive test skipped with `DUALPOOL_V738_ARCHIVE` unset. No v7.3.8 archive was downloaded/executed and no product/provider/credential/live mutation occurred. Evidence/report: [`ACL seam evidence`](../evidence/phase-2-v7.3.8-stage-install/acl-seam-repair.json), [`repair report`](reports/2026-09-22T1303Z-phase-2-v738-stage-install-acl-seam-repair.md). JSON/link/privacy/secret gates passed. Repair commit `ae9cd3350aeb5b0a6937f14b254706cedc1132b0` was pushed normally; exact Source CI run [35733252908](https://github.com/trungqwe/dual-pool/actions/runs/35733252908) completed successfully for that exact SHA and branch. A documentation/evidence-only receipt update follows; its push-triggered CI must also pass before this run ends. Do not start Smoke or move upstream. After successful delivery, the sole next objective is owner/auditor review and controlled delivery of the repaired v7.3.8 stage/install lineage into `phase-2/upstream-lifecycle`.
# Current Handoff
...
Implementation/evidence commit `11ae515929031b5f03305ff49bc882409751c228` was pushed normally to `origin/phase-2/v7.3.8-candidate-stage-install`. Exact Source CI run [`35719533512`](https://github.com/trungqwe/dual-pool/actions/runs/35719533512) was `push / completed / success` for that exact branch and SHA; Go format, module verify, vet, tests, race, pinned integration, Windows build, Phase 0 regressions and candidate-lock validation all passed. A follow-up delivery-only handoff commit records this receipt. Do not move `phase-2/upstream-lifecycle` from `5bfb1518059a171a0f3858877585e3097d9fe099`. Production Smoke, product-root candidate installation, promotion/rollback, retained-handle launch, TOCTOU, process/OS/power-loss durability and updater CLI remain OPEN.
...
v7.3.8 provenance/config verification đã được delivery. TEMP cleanup classification là `POLICY_BLOCKED_NONREPO_TEMP_RESIDUE`, không phải delivery blocker: automatic policy từ chối deletion, không có bypass; residual chỉ nằm trong system TEMP ngoài repository, product root và các user config roots đã kiểm tra. Candidate PID `19432` đã dừng; loopback port `60422` free; không có auth files trong TEMP harness dirs, không có v7.3.8 match trong `DualPool/bin`, không có repo binary/TEMP artifact hay staged file. Synthetic client/management keys chỉ dùng cho disposable verification, không đọc từ secretstore/WinCred, không dùng provider credential; không ghi raw keys vào evidence. Không có production mutation. Evidence: `evidence/phase-2-v7.3.8-verification/security-gate.json`.

Commit `918722fe91995436a9ffd24b8684ca210748f0e9` được push; run `35702682125` thất bại ở Go tests vì fixture newline của tamper test không mutate receipt trên checkout CRLF. Commit sửa test `319e135c5888c3d41730c8eeb284316b3ca49706` đã push; exact-SHA Source CI `35703548549` hoàn tất `success`, toàn bộ source-checks PASS. Branch HEAD và origin cùng ở `319e135c5888c3d41730c8eeb284316b3ca49706`; `phase-2/upstream-lifecycle` vẫn ở `a0b1573346007ad08a9ebe7012167bffa79fbd99`. Delivery đủ điều kiện cho independent audit nhưng chưa được independent acceptance. TEMP residue vẫn `POLICY_BLOCKED_NONREPO_TEMP_RESIDUE`, không xóa/bypass lại. Các gate product staging/Smoke/promotion/rollback/handle/TOCTOU/OS durability/updater CLI vẫn OPEN. Không bắt đầu registry hay candidate staging trong slice này.

Delivery receipt report: [2026-09-22 v7.3.8 delivery receipt](reports/2026-09-22T0825Z-phase-2-v738-delivery-receipt.md). The earlier implementation report remains immutable as committed.
...
## Phase 2 — production compatibility Smoke v7.3.8 (2026-09-22)

Slice implementation starts at `4d56626a361b10abfdef63d898a59267f1f6a84d` on branch `phase-2/v7.3.8-production-smoke`; `phase-2/upstream-lifecycle` remains unchanged. Manager-owned `update.Smoke` now validates the closed trusted v7.3.7/v7.3.8 authority, runs disposable loopback-only compatibility checks with synthetic keys and exact installed-slot identity, and reuses the production readiness contract for candidate pools. Transaction regressions cover pre-Smoke immutability, rollback, recovery and marker retention. Evidence: `evidence/phase-2-v7.3.8-production-smoke/`; report: `docs/reports/2026-09-22T1702Z-phase-2-v738-production-smoke.md`. The opt-in real archive test was skipped because `DUALPOOL_V738_ARCHIVE` was not supplied. No live product/provider mutation or promotion occurred. Production-root acceptance, real promotion/rollback, retained launch handle, TOCTOU, OS/power-loss durability and updater CLI remain open.
