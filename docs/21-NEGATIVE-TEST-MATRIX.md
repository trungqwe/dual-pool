...
Independent-audit repair adds deterministic negative coverage for canonical-marker no-replace, exact-handle path replacement blocking, transient/permanent sharing contention, delete without `DELETE` access, exact-object disposition, partial/unexpected running sets, and duplicate-start avoidance. These are E3 synthetic Windows/TEMP proofs; production updater acceptance remains OPEN.
