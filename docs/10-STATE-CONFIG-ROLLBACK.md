...
Lock ownership uses PID, the raw 64-bit Windows process creation FILETIME, canonical executable image and an opaque operation ID. Expiry is diagnostic metadata and never permits stealing a lock from an exact live owner. A successful Guard retains an exclusive Windows handle opened with `GENERIC_READ | DELETE`, share mode `0`, `OPEN_EXISTING` and `FILE_FLAG_OPEN_REPARSE_POINT`. Stale inspection and its repeated owner check operate through that same handle; intentional release or proven stale reclamation uses `SetFileInformationByHandle(FileDispositionInfo)` so deletion targets the inspected file object. Canonical locks are never deleted by pathname. Normal ownership does not use delete-on-close, so a process crash leaves metadata for verified recovery. An unverifiable owner or invalid record fails closed.
...
## Phase 1 synthetic Codex config transaction engine

`internal/configtxn` implements the fixture-only Codex TOML transaction boundary. It acquires `GLOBAL`, creates and verifies an exact backup and sibling candidate, syncs a strict recovery marker, persists ownership as `PENDING`, then acquires the target lock for the final hash CAS and `ReplaceFileW`. After hash and semantic verification plus target sync, it releases the target lock, finalizes ownership as `APPLIED`, and removes the marker.

The document is validated with pinned `github.com/pelletier/go-toml/v2 v2.4.3`; no serializer is used. A constrained internal span locator accepts canonical top-level basic strings and the exact regular provider table. Existing values are changed only inside their quoted literal spans. Missing scalars are inserted before the first table, and a missing provider table is appended with one owned separator newline. UTF-8 with optional BOM and uniform LF/CRLF is preserved; mixed newlines, invalid UTF-8, files over 1 MiB, aliases, array tables, nested or unexpected provider fields fail closed.

The production registry in this component is limited to `model`, `model_provider`, and `model_providers.dualpool_codex`. `model_catalog_json` exists only behind `AuthorizedCatalogFallback`, which represents a separately completed exact-version gate and is disabled by default. No Antigravity production adapter exists.

Recovery derives PRE/POST from hashes. Apply PRE removes pending intent and its transient artifacts; Apply POST validates owned semantics and finalizes `APPLIED`; a missing or corrupt target is restored from the verified pre-install backup; valid unknown drift conflicts. Rollback requires current owned semantics to match the recorded applied values, surgically restores original value or absence while retaining unrelated edits, and marks `ROLLED_BACK`. Rollback PRE retains `APPLIED`; rollback POST finalizes `ROLLED_BACK`; unknown drift remains diagnostic. The original file identity records the pre-replacement object because `ReplaceFileW` installs the replacement object's identity.

This fixture proof does not establish product-root or backup DACL acceptance. Real Codex configuration mutation remains NO-GO until that boundary is separately accepted.
