...
Recovery dùng marker trước candidate chứa secret, creation-time protected DACL cho file, `MoveFileEx` không replace trên cùng volume. Marker/candidate lạ hoặc final sai schema/ACL/key đều fail closed. Không chạy binary/listener. Chưa chạy real generation; nó chỉ được mở sau implementation Source CI PASS. Phase 2 vẫn OPEN.
