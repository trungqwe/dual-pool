...
| U-008 | CLIProxyAPI health and provider-specific endpoint shapes for pinned release. | **PARTIAL / UNKNOWN** — `v7.3.7` integrity, config fields, registered routes, loopback bind and key enforcement were probed in `cliproxy-schema.json`; Pinned Management `/auth-files` authenticated empty-inventory response shape is PROBED; provider-specific response shapes, dedicated provider health semantics, and credentialed provider health remain unproven. | Credentialed provider contract probes in a later authorized run. | Adapt through documented version surface or pin another tested release. |
...
The authorized primary-profile attempt was then run with temporary user-level config mutation under an external watchdog. The authoritative retry restored `config.toml` byte-for-byte but failed at `NORMAL_RELAUNCH_FAILED` before authentication/Astra/wire checkpoints. U-006 remains **BLOCKED**; no target request was captured. See [primary-profile report](../docs/reports/2026-09-19T0715Z-phase-0b-u006-primary-profile.md) and [audit correction](../evidence/phase-0b-u006-primary-profile/primary-profile-audit-correction.json).

## Phase 0 closure — 2026-09-19

Phase 0 is closed as **PASS_WITH_BLOCKERS** under the roadmap exit rule. Phase 1 fixture-only foundation work is **GO**. Live Antigravity integration, Codex picker integration, and release promotion remain **NO-GO** while U-001..004 and U-006 are blocked and U-008 remains partial/unknown. CLIProxyAPI `v7.3.7` is recorded in `upstream.lock` as a supply-chain and credential-free lifecycle **candidate**, not a production-supported provider adapter.
