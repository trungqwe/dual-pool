...
| U-005 | Installed Codex version contains Astra and extension shares the intended config layer. | **PARTIAL / UNKNOWN** — CLI transport remains provisional because v3 Gate A failed process-termination completion; extension sharing is unproven. Current status: `phase-0b-status-v3.json`. | Repair P0B-CX-TRANSPORT-001, then user-assisted extension/config-layer and outbound Responses capture. | Upgrade supported client or exact-version fallback; no main-branch catalog. |
| U-006 | Model picker retains `model_provider=dualpool_codex` when choosing Astra. | **BLOCKED** — user-assisted picker selection is required and automatic Codex extension UI control is prohibited. | User selects Astra in the actual extension picker and request arrives on Codex loopback port. | Use a supported profile/default workflow or block dropdown acceptance; do not patch UI. |
