...
| U-005 | Installed Codex version contains Astra and extension shares the intended config layer. | **PARTIAL / UNKNOWN** — Gate A v5 PASS trên Codex CLI 0.154.0; extension/config-layer chưa được chứng minh. [Trạng thái hiện hành](../evidence/phase-0b-v5/phase-0b-status-v5.json). | User-assisted extension/config-layer và picker request tới loopback trong run riêng. | Upgrade supported client or exact-version fallback; no main-branch catalog. |
