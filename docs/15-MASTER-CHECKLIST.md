...
- [x] Phase 2 updater lifecycle authority is sealed: no public raw locked lifecycle factory, composition derives every transaction authority from Manager, injected dependencies cannot bypass layout validation, and all runtime participants share one lock manager rooted at `layout.Locks`. Evidence: `docs/reports/2026-09-21T2037Z-phase-2-updater-authority-binding-repair.md`, `evidence/phase-2-updater-manager-composition/composition.json`.
